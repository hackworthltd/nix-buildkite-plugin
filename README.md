...
`nix-buildkite-plugin` is a Buildkite plugin that evaluates Nix
expressions which create derivations, analyses these derivations to
find a topological ordering that will ensure steps have the correct
dependencies between them, and then creates an ordered Buildkite
pipeline in which each step corresponds to one of these derivations.
...
To use this plugin in your repo, add a file named
`.buildkite/pipeline.yml` (or similar) which includes a step that
invokes the plugin. The plugin will use
...
evaluated, will produce a list of derivations to build:
...
for details, but your static Buildkite pipeline should look something
like this:
...
If `true`, the plugin will instead generate a Buildkite pipeline that uses `nix build` to build each derivation's corresponding Nix attribute. `nix build` is generally more flexible than `nix-store`, so you may prefer this mode. However, see the FAQ below for a discussion of the performance trade-offs between the two modes.
...
The plugin uses `jq` to convert the JSON output of `nix-eval-jobs` to a list of derivations and their metadata. The default `jq` filter used to generate this list from the JSON generated by `nix-eval-jobs` depends on the value of the `use-nix-build` option:

- if `false`, the filter is `try .drvPath catch halt_error`
- if `true`, the filter is `try ([.drvPath, .attr] | join(" ")) catch halt_error`

You can change the filter that the plugin passes to `jq` via this plugin option. For example, assuming your flake's `hydraJobs` has a `required` attribute that aggregates a number of other flake outputs, you may want to add the job's `constituents` to the list of derivations to build:
...
The plugin runs `jq -re` by default, but you can change the options provided to `jq` via this plugin option.
